...
export default function useStaking(stakeNodes, list) {
  ...
  const getDelegationFee = async (txType, validatorAddr, validatorSrcAddr = '') => {
    ...
    const stakeAccount = Array.isArray(validatorAddr) ? validatorAddr[0]?.holderAccount || validatorAddr[0]?.holderAccount : validatorAddr?.holderAccount || validatorAddr?.holderAccount
    ...
    const { ok, resFee, maxAmount, resAdding, enough } = await currentWallet.value.getDelegationFee({
      ...
      btcAccount: currentWallet.value.btcAddress || null,
    ...
    if (mode.value === 'redelegate' || activeTab.value === 'redelegate') {
      ...
      return {
        ...
        desc: currentWallet.value.unstakePerioudFrom ?
          ...
          : currentWallet.value.net === 'solana' ? 'Select validators and enter the amount to unstake. Unstaking may take up to 2 days.' : `${t(currentWallet.value.net === 'stacks' ? 'staking.chooseNodeModalDescStacks1' : /* currentWallet.value.messages.unstakeingPrefix ? currentWallet.value.messages.unstakeingPrefix : */ 'unstaking.defaultPrefix')} ${currentWallet.value.net === 'stacks' ? '' : t('unstaking.chooseNodeModalDesc', {
            net: currentWallet.value.name,
            perioud: currentWallet.value.unstakeingPerioud,
          })}`,
        list: list(),
